...
export default function App() {
  const { user, loading, dealError, dealErrorReason, dealing, retryDeal } = useAuth();
  ...
  // deal + error state.
  //
  // #434: on a CONNECTION-class deal failure, PREFER this device's durable card
  // snapshot over the full-screen reload screen. A Player who was already dealt
  // in still sees their card (read-only, refreshing in the background via Retry)
  // instead of a dead-end — the exact "it should be cached and load in the
  ...
  // paints a real card.
  //
  // Gated on `dealErrorReason === 'connection'` so the fallback NEVER hides an
  // actionable error (Codex P2, #438): a pool-shortfall keeps its own DealError
  // ("ask an admin to add prompts", which PoolRecoveryWatcher also auto-recovers
  // on the reason). This mirrors the #403 swallow, which excludes pool-shortfall
  // from the cached-card path for the same reason. The full DealError also stays
  // for a genuine first-timer with nothing cached.
  //
  ...
  const cachedCard = dealError && dealErrorReason === 'connection' ? loadCardSnapshot(user.uid) : null;
