...
describe('tabs contract (frozen stable mount points)', () => {
  it('defines exactly the five Card/Feed/Ranks/Prompts/Admin tabs, in order', () => {
  ...
  it('marks only the Admin tab as adminOnly', () => {
    for (const tab of TABS) {
      expect(tab.adminOnly).toBe(tab.id === 'admin' ? true : undefined);
    }
  });
...
describe('visibleTabs', () => {
  it('hides the Admin tab for a non-admin Player', () => {
    expect(visibleTabs(false).map((t) => t.id)).toEqual(['card', 'feed', 'ranks', 'prompts']);
  });

  it('shows the Admin tab for an Admin', () => {
    expect(visibleTabs(true).map((t) => t.id)).toEqual(['card', 'feed', 'ranks', 'prompts', 'admin']);
  });
});
...
describe('TabBar (real render — no Firebase-backed hooks in this component)', () => {
  it('renders Card/Feed/Ranks/Prompts and hides Admin for a non-admin Player', () => {
    const html = renderToStaticMarkup(
      <MemoryRouter>
        <TabBar isAdmin={false} />
      </MemoryRouter>,
    );
...
describe('route table (mirrors App.tsx\'s TABS -> <Route> mapping)', () => {
  ...
  it('mounts none of the five known tabs for an unrecognized path (defers to the "/" redirect)', () => {
