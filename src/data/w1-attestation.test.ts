...
describe('attestAdult persists the 18+ self-attestation create-only (#23)', () => {
  it('attesting persists the timestamp on a profile that has none', async () => {
    const tx = driveTransaction(snap({ displayName: 'Ada', createdAt: 1 }));
    await attestAdult('sailor-1', 1_720_000_000_000);
  ...
  it('creates the stamp when the profile row does not exist yet', async () => {
    const tx = driveTransaction(snap(null));
    await attestAdult('sailor-1', 42);
    expect(tx.set).toHaveBeenCalledWith(
      { path: 'users/sailor-1' },
      { attestedAdultAt: 42 },
      { merge: true },
  ...
  it('never overwrites an existing EARLIER attestation', async () => {
    ...
    await attestAdult('sailor-1', 999);
