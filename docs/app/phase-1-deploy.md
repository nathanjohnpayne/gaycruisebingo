...
**`moderateProof` (Cloud Vision) is gated OFF by default (issue #126).** `moderateProof` is a `us-central1` function but the project's default Storage bucket is `us-east1` — a region mismatch that fails Firebase's deploy-plan validation, and Firebase validates every trigger in the module at plan time, so an unresolved `moderateProof` blocks deploying the whole `functions` package, including the notifiers above. The `ENABLE_VISION_MODERATION` flag (in `functions/.env` / `functions/.env.<projectId>`, default `false`) controls whether `moderateProof` is exported at all: off, it is not exported and Firebase never sees or validates it, so this deploy brings up the notifiers (and the #43 threshold-hide function once it lands) but **not** Cloud Vision.
...
To enable Vision later: (1) enable the Cloud Vision API on the project, (2) resolve the region mismatch (`moderateProof` must run in the bucket's region, `us-east1`), (3) set `ENABLE_VISION_MODERATION=true` in `functions/.env.<projectId>`, and (4) redeploy `--only functions`.
