...
- The fix: `setMark` folds onto `getDocFromCache(boardRef)` instead of trusting the caller's prop. That is a **cache-only read — no server round trip, still fully offline-capable** — and it sees this client's own just-applied mutation (and, via the shared persistent multi-tab cache, another of the owner's tabs' already-synced ones) far sooner than a cross-process `onSnapshot` re-render can. It falls back to the caller-supplied `cells` only when nothing is cached yet (the very first local knowledge of the Board — never reachable in production, where `Board.tsx` only renders Marks once a Board has already loaded). `firstBingoAt` is still taken from the caller's live `currentFirstBingoAt` (the `useMyPlayer` value), not a cache read of the Player row — a residual, low-severity edge: two rapid Marks that both stand up a BINGO could stamp `firstBingoAt` a few milliseconds late rather than exactly at the first one. Acceptable per ADR 0001 ("for-fun tallies, not tamper-proof records") and far narrower than the cells-clobbering bug it replaced.
- Honest residual scope: `getDocFromCache` closes the race for the dominant real case (repeated taps in one tab, and same-process multi-tab, since the persistent-cache read reflects a just-applied local mutation well before any cross-tab `onSnapshot` echo would). A true simultaneous write from two *separate* tabs/devices, timed within the multi-tab cache's own propagation window, is not fully eliminated — closing that completely would need a transactional (server-round-trip, non-offline-queueable) read, which is exactly the property this rework trades away. Given ADR 0001's non-adversarial, single-Player-usually-single-tab threat model, that residual is accepted rather than hidden behind an overclaim.
