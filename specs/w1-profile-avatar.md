...
- **Given** a signed-in User types a new display name **when** `updateDisplayName(uid, name)` is called **then** it trims the name, caps it at `MAX_DISPLAY_NAME` (40 characters), and writes `users/{uid}.displayName`; a blank/whitespace-only name is a no-op so a cleared input can't wipe out the existing name. (Test: "trims and persists the display name, caps it at 40 characters, and no-ops on blank".)
