...
- `src/data/dayMeta.ts` (new) — `pinDayFirstBingo(dayIndex, who, at)` write-once per-Day honor at `days/{dayIndex}/meta/{dayIndex}`, and `broadcastDayBlackout(who, dayIndex)` — the per-card blackout Moment (deterministic id `${uid}-blackout`, immutable per rules) that NAMES its Day via `dayIndex`. Both mirror `moments.ts`'s offline-queueable, cache-pre-checked, fire-and-forget writes.
...
- `src/data/d15-scoring-aggregates.test.ts` (Vitest unit) — `computeMark` → `foldDayStat` writes `dayStats[dayIndex]` and leaves siblings untouched; `setMark`'s player write carries the folded `dayStats` + summed root; `dayMeta.ts` `pinDayFirstBingo`/`broadcastDayBlackout` cache-pre-check and payload shape.
