...
- `src/data/api.ts` — a small, self-contained rate-limit helper (`checkItemRateLimit` + the `ITEM_RATE_LIMIT_MS` constant it's judged against) is added next to `addItem`/`reportItem`. Neither `addItem` nor `reportItem` changed behavior; the guard lives one layer up, at their one real call site — see "Why the guard lives in the caller, not the write" below.
...
While throttled, the relevant control (`Add`, or a `Report` button) is `disabled`, and a `role="alert"` message appears ("Slow down — you can add/report again in a few seconds."). Both states clear on their own once `ITEM_RATE_LIMIT_MS` elapses, via a real `setTimeout` scheduled at the moment the guard first blocks an attempt — not a live countdown, just a single fire-and-forget timer per throttle episode, cancelled and rescheduled if a new blocked attempt lands first and cleared on unmount so a Player who navigates away from Prompts mid-window never triggers a `setState` after unmount.
...
- Given a Player adds/reports rapidly, when they exceed the client rate-limit, then further add/report is throttled (disabled control + human message) until the window passes — `src/components/w1-prompt-pool.test.tsx` (both throttle tests).
