...
`isDoubtSatisfied(doubt, itemText, proofs)` is `proofs.some(p => p.uid === doubt.targetUid && p.itemText === itemText && p.createdAt >= doubt.createdAt)` — "at or after", so a Proof answering a Doubt (always created after it) satisfies, while a Proof that predated the Doubt does not (the Doubt asks for FRESH pics). `openDoubts` is the unanswered subset (its `.length` is the single per-Prompt open count both surfaces read), and `doubtStatusFor(uid, …)` collapses a Player's Doubts to `none`/`open`/`satisfied`, with `open` winning any mix.
...
- **One count, two surfaces** (ADR 0002): the Square badge and the Tally-sheet header both read `openDoubts(...).length` for the same Prompt.
...
- The open-Doubt count renders on a marked, non-free Square (the DoubtBadge), and disappears once every Doubt for the Prompt is answered by a Proof (derived live).
...
- An answered Doubt renders a DISTINCT satisfied state from an open one, and the header summary counts only the open Doubts.
```
