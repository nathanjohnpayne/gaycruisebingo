...
- `src/components/SignIn.tsx` exports `DealError`, the retry surface: it shows the message with `role="alert"` and a Retry button that calls `onRetry`, disabled and relabeled while `retrying`. (`DealError` additionally hosts the pool-recovery auto-retry watcher — it is the surface mounted while the error is up — specified and tested by `specs/w1-board-deal-join.md`.)
