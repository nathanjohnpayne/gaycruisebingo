...
- `src/auth/AuthContext.tsx` owns the post-sign-in deal. It runs `joinAndDeal` when the User becomes known, fires `track('join_event')` on success, and exposes `dealError: string | null`, `dealing: boolean`, and `retryDeal(): void`. `signIn` first sends a Player who is on a Firebase alias origin (`gaycruisebingo.web.app` / `.firebaseapp.com`) to the canonical origin `gaycruisebingo.com` via a history-replacing navigation (#162), so Google sign-in runs same-origin with the OAuth handler (`authDomain = gaycruisebingo.com`, #161) rather than failing with "missing initial state" in storage-partitioned webviews; the redirect is gated on the sign-in action, not boot, so a signed-in Player reading a cached board offline is never navigated away. On the canonical origin (and in dev) `signIn` opens the Google popup and fires `track('login', { method: 'google' })` on success, or on failure fires `track('login_failed', { method: 'google', code, message })` (the Firebase error code/message, PII-free) and rethrows to preserve the caller contract (#163). Google remains the only provider.
...
- Given a Player is on a Firebase alias origin (`gaycruisebingo.web.app` / `.firebaseapp.com`), when they tap sign in, then `signIn` navigates (history-replacing) to the same path on `gaycruisebingo.com` and does NOT open the popup on the alias origin, so authentication happens same-origin with the OAuth handler.
...
- `signIn` fires `track('login', { method: 'google' })` on success; on a popup rejection it fires `track('login_failed', …)` with the Firebase code and rethrows (no `login`, no attestation); and from a Firebase alias origin it history-replaces to `gaycruisebingo.com` without opening the popup.
