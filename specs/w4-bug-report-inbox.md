...
Opening the control captures only the rendered `.app` surface while excluding the reporting UI. If full-surface capture fails, the client retries with a compatibility pass that lowers pixel ratio and excludes embedded media surfaces that desktop Safari and stale canvases commonly cannot serialize. The sheet explains which diagnostics are sent and previews the screenshot. A capture failure never discards the description: the Player can retry capture or submit text only. Submission includes bounded description, current app-relative route, event id, deployed commit, user agent, viewport, online state, optional capture error, and optional PNG. It includes no email, token, console log, local-storage value, proof URL, browser chrome, or other application.
