...
- **Edit** (#455) — swaps the row for an inline editor: a title input and body textarea prefilled with the current copy, under the same caps as Compose, with `Save` / `Cancel`. Save is disabled while either field is blank; on a settled success the editor closes; a rejected save KEEPS the editor open with the draft intact and surfaces an inline `role="alert"` (the same #411 convention Compose uses), so a retry is one tap. Only copy is editable — the byline, Day, and Feed position are fixed here and in the rules.
