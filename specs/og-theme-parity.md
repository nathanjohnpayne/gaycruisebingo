...
- **Given** the five shared keys (`bg`/`ink`/`primary`/`secondary`/`accent`) **when** any hex value differs between the two sources for any theme **then** the guard names the drifted theme and key. The `glamiators` accent is resynced to `#fffefb` by this change; `summer-white` already matches main (#63 is unmerged, so its deepened values are not canon yet). (Test: "matches themes.css on every shared palette key for every theme".)
