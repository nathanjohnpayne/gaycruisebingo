...
3. **Open-time reconcile** (`reconcileEchoes`, called by `Board` once per board identity per session): reconciles the opened board against the achieved set and writes any missing echoes — the lazy backfill that self-heals pre-feature boards without a migration script and mops up any echo write dropped offline.
