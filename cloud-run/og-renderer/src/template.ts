...
//
// THEMES duplicates src/theme/themes.css because this service's build context
// is this directory only (Dockerfile copies just src/), so it can never read
// the app's CSS. The duplication is pinned by src/og-theme-parity.test.ts in
// the app suite: any drift between the two sources fails `npm test` / app-ci.
// Edit both files together. (Scaffold-lifetime guard — #39 deletes both.)
...
const THEMES: Record<string, Palette> = {
  ...
  glamiators: { bg: '#0c0a08', ink: '#fff7e8', primary: '#e8c86a', secondary: '#c9a24a', accent: '#fffefb' },
  'summer-white': { bg: '#f6f1e7', ink: '#20170f', primary: '#d9a441', secondary: '#c48a2c', accent: '#20170f' },
...
export function renderHtml(opts: { kind: string; title: string; subtitle: string; theme: string }): string {
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><style>
    ...
    *{margin:0;box-sizing:border-box}
    ...
    .title{font-family:'Bebas Neue';font-size:118px;line-height:.95;margin:10px 40px;color:${p.ink};
